...
import { assertMagicBytesMatch, assertMimeAllowed, assertSizeWithinLimit } from '../../domain/entities/stored-file.entity';
...
export function prepareMultipartUploads(request: FastifyRequest, reply: FastifyReply): Promise<PreparedMultipartUpload[]> {
  ...
  const cleanup = (): void => {
    ...
    void result.catch(() => undefined).then(async () => {
      for (const path of paths) {
        await unlink(path).catch((error: unknown) => request.log.error({ err: error }, 'Upload temporary file cleanup failed'));
      }
    });
  };
...
export async function prepareMultipartUpload(request: FastifyRequest, reply: FastifyReply): Promise<PreparedMultipartUpload> {
...
function acquireRequest(request: FastifyRequest, reply: FastifyReply): AbortSignal {
  ...
  if (activeRequests >= maximum) throw new HttpException('Upload capacity reached. Retry shortly.', 429);
  activeRequests += 1;
...
async function prepare(request: FastifyRequest, reply: FastifyReply, paths: string[]): Promise<PreparedMultipartUpload[]> {
  ...
  const abort = (): void => { request.raw.destroy(); };
  signal.addEventListener('abort', abort, { once: true });
  try {
    for await (const part of request.parts({ limits: UNCAPPED_PART_LIMITS(maximum) })) {
      signal.throwIfAborted();
      if (part.type !== 'file' || part.fieldname !== 'file') throw new BadRequestException('Only file fields named file are accepted.');
      if (files.length >= maxFiles) throw new PayloadTooLargeException('Upload exceeds the file count limit.');
    ...
    if (error instanceof Error && 'statusCode' in error && 'code' in error && typeof error.code === 'string' && error.code.startsWith('FST_') && typeof error.statusCode === 'number') {
...
async function saveFile(part: MultipartFile, options: {
  filepath: string;
  maximum: number;
  ...
  await pipeline(part.file, inspect, createWriteStream(options.filepath, { flags: 'wx', mode: 0o600 }), { signal: options.signal });
  ...
  assertMagicBytesMatch(signature.subarray(0, Math.min(size, SIGNATURE_BYTES)), part.mimetype, 'file');
  return { filepath: options.filepath, contentType: part.mimetype, size, digest: hash.digest('hex'), signal: options.signal };
}
