...
function payload(boundary: string, contents: Buffer[]): Buffer {
  return Buffer.concat(contents.flatMap((body) => [
...
async function app() {
  ...
  server.setErrorHandler((error: unknown, _request, reply) => {
    const status = error instanceof Error && 'getStatus' in error && typeof error.getStatus === 'function' ? error.getStatus() as number : 500;
...
describe('prepareMultipartUploads', () => {
  it('streams multiple files and cleans temporary files after response', async () => {
    ...
    try {
      const response = await server.inject({ method: 'POST', url: '/', headers: { 'content-type': 'multipart/form-data; boundary=sample' }, payload: payload('sample', [PNG, PNG]) });
      expect(response.statusCode).toBe(200);
      const files = response.json<{ size: number; digest: string; filepath: string; body: string }[]>();
      expect(files).toHaveLength(2);
      expect(files[0]).toMatchObject({ size: PNG.length, digest: createHash('sha256').update(PNG).digest('hex'), body: PNG.toString('hex') });
  ...
  it('rejects rather than hangs when the plugin itself caps the file count', async () => {
    vi.stubEnv('UPLOAD_MAX_FILES', '1');
    const server = Fastify();
    await server.register(multipart, { limits: { fileSize: 10 * 1024 * 1024, files: 1, parts: 1 } });
    server.setErrorHandler((error: unknown, _request, reply) => {
      const status = error instanceof Error && 'getStatus' in error && typeof error.getStatus === 'function' ? error.getStatus() as number : 500;
      void reply.status(status).send({ status });
    });
    server.post('/', async (request, reply) => (await prepareMultipartUploads(request, reply)).length);
    try {
      const response = await server.inject({ method: 'POST', url: '/', headers: { 'content-type': 'multipart/form-data; boundary=sample' }, payload: payload('sample', [PNG, PNG]) });
