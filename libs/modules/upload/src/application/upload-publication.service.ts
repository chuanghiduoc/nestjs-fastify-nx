...
import { STORAGE_PORT, type StoragePort, type StoredFile as StoredFileResult } from '@nestjs-fastify-nx/infra-storage';
import { StoredFile } from '../domain/entities/stored-file.entity';
import { STORED_FILE_REPOSITORY, type StoredFileRepositoryPort } from '../domain/ports/stored-file-repository.port';
import { UPLOAD_VERIFICATION_DISPATCHER, type UploadVerificationDispatcher } from './ports/upload-verification.dispatcher';
...
export class UploadPublicationService {
  ...
  constructor(
    ...
    @Inject(UPLOAD_VERIFICATION_DISPATCHER) private readonly verification: UploadVerificationDispatcher,
  ) {}
  ...
  async result(file: StoredFile, correlationId?: string): Promise<StoredFileResult> {
    if (file.status === STORED_FILE_STATUS.VERIFYING) {
      await this.verification.dispatch({
        ...
        this.logger.error({ err, fileId: file.id }, 'Upload verification enqueue failed; status polling will retry');
      });
    }
    return {
      id: file.id,
      ...
      size: file.size,
      status: file.status,
      url: file.status === STORED_FILE_STATUS.READY
