...
import { ERROR_CODES, fieldProblem, I18N_KEYS } from '@nestjs-fastify-nx/contracts';
import {
  ALLOWED_MIME_TYPES,
  detectFileType,
  MIME_EXTENSIONS,
...
export class StoredFile {
  ...
  withStatus(status: StoredFileStatus): StoredFile {
    return new StoredFile({ ...this.props, status });
  }
}

function mimeNotAllowed(contentType: string, path: string): DomainException {
  return new DomainException(
    fieldProblem({
      kind: 'validation',
      code: ERROR_CODES.UPLOAD_MIME_NOT_ALLOWED,
      violationCode: 'mime_not_allowed',
...
export function assertSizeWithinLimit(size: number, maxBytes: number): void {
  ...
  throw new DomainException(
    fieldProblem({
      ...
      violationCode: 'size_out_of_range',
  ...
  if (!detected) {
    throw new DomainException(
      fieldProblem({
        ...
        violationCode: 'magic_bytes_unknown',
  ...
  if (detected.mimeType !== declaredContentType) {
    throw new DomainException(
      fieldProblem({
        ...
        violationCode: 'magic_bytes_mismatch',
...
export function objectNotFound(key: string): DomainException {
  return new DomainException(
    fieldProblem({
      ...
      violationCode: 'object_not_found',
