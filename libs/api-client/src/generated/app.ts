 ...
 */
import type { ServiceInfoDto } from './api.schemas';

import { customAxiosInstance } from '../lib/axios-instance';

export const getApp = () => {
  /**
   * @summary Service metadata (name + version).
   */
  const appRoot = () => {
    return customAxiosInstance<ServiceInfoDto>({ url: `/api/v1`, method: 'GET' });
  };
  return { appRoot };
};
export type AppRootResult = NonNullable<Awaited<ReturnType<ReturnType<typeof getApp>['appRoot']>>>;
