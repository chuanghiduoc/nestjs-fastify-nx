...
- **Prisma errors have a global safety-net in `GlobalExceptionFilter`** (`PRISMA_ERROR_MAP`): unhandled `PrismaClientKnownRequestError` maps P2002/P2003→409, P2025→404, P2023→400, P2028/P2024→503 instead of a blanket 500. The raw Prisma message (table/column/constraint names) is NEVER sent to the client — only status/code/title go out, full error is logged. It is a SAFETY-NET: prefer explicit `BusinessRuleException`/domain handling; unmapped Prisma codes still fall through to a redacted 500.
