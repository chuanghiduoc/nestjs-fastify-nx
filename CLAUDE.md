...
Respond to the maintainer in chat in **Vietnamese by default** — reviews, explanations, and status reports — unless they write in another language or ask otherwise. Code, identifiers, commit messages, comments, and committed documentation stay in English.
...
vitest.shared.ts    Shared vitest factory — all project configs import from here (coverage thresholds, setup, excludes)
```
...
application/
  ...
  ports/           driven ports the APPLICATION owns rather than the domain —
                   an outbound capability with no domain meaning (queue
                   dispatcher, mailer). A port the domain reasons about
                   (repository) belongs in domain/ports/ instead.
  dto/             application transport types — PURE TS, no @nestjs/swagger
...
infrastructure/
  ...
  dispatchers/     adapters of application/ports (BullMQ, SMTP, …)
presentation/
...
testing/           in-module test doubles (mock repository, entity factory)
                   shared across this module's specs. NOT the Testcontainers
                   harness — that is @nestjs-fastify-nx/testing.
<context>.module.ts            full slice: controllers + handlers + adapters (api hosts this)
<context>-listeners.module.ts  listeners-only slice for hosts that must NOT load the
                               HTTP surface (worker, scheduler). Add it with the module's
                               first listener; the generator does not scaffold one because
                               an empty listeners module is dead code.
index.ts           public barrel — re-export only what consumers need
```

**Folder naming is plural for collections of peers (`entities/`, `queries/`, `repositories/`) and singular for `dto/`.** `dto/` is singular in every layer — `application/dto/`, `presentation/dto/`, `libs/contracts/src/lib/dto/`, `apps/api/src/graphql/dto/`. Do not reintroduce `dtos/`.

**CQRS bus (`@nestjs/cqrs`)**: command/query handlers are decorated with `@CommandHandler`/`@QueryHandler` and registered automatically by `CqrsModule.forRoot()` (imported once per runnable app: `api`, `worker`, `scheduler`, `codegen-app` — the worker needs it because its upload processor dispatches the shared `VerifyUploadCommand` rather than reimplementing the lifecycle). Consumers (controllers, GraphQL resolvers, event listeners) inject `CommandBus`/`QueryBus` and dispatch `commandBus.execute(new SomeCommand(...))` / `queryBus.execute(new SomeQuery(...))` — never inject a handler directly. Queries/commands extend `Query<TResult>`/`Command<TResult>` so `execute()` infers the return type; the result interface is co-located in the query/command file. The barrel exports the query/command classes + result types, **not** the handlers (handlers stay internal, found by the bus explorer). Domain events still flow through the **outbox** (`EventPublisherPort` + `OutboxRelayService`), NOT the in-memory `@nestjs/cqrs` EventBus/AggregateRoot — the outbox is the transactional, cross-process channel and must not be replaced by in-memory event publishing.
...
2. **Comments — none. New code ships with zero comments.** Not "few", not "only the important ones": zero. The code must carry its meaning through naming, small functions, explicit types and tests. Anything that cannot be expressed that way is a _decision_, and decisions belong in `docs/adr/`, indexed by `docs/adr/README.md`. This applies to `//`, `/* */`, JSDoc and TODO alike. Pre-existing comments in code untouched by a change stay where they are; do not run comment-stripping sweeps as a side effect of unrelated work.
...
7. **DTOs**: validated with `class-validator`, transformed with `class-transformer`, documented with `@nestjs/swagger` decorators. Re-export from module barrel only what consumers (composition libs, controllers) need — keep internals private.
   7b. **Validation is declarative, never hand-rolled.** Validate input with pipes and decorators, not `if`/`else`/regex scattered in handlers. Path params that are ids MUST use `@Param('id', new ParseUUIDPipe({ version: '7' }))` — this repo's ids are UUID v7, and a bare `ParseUUIDPipe` (accepts ANY version, so a wrong-version id like a v4 passes) or a plain `@Param('id') id: string` both let a wrong-version id reach the handler. The module generator template already scaffolds this on its `:id` route — keep it. Body validation lives on the presentation DTO (class-validator decorators); reserve `if`/`else` for business/authorization logic (e.g. per-user ownership checks) that no decorator can express. Authenticated routes read the caller via `@CurrentUser() user: AuthenticatedSession` from `@nestjs-fastify-nx/infra-auth` — never re-cast `@Req() req: FastifyRequest & { user }`.
8. **Module boundaries are sacred** — if lint fails on `@nx/enforce-module-boundaries`, fix the architecture, do not relax the rule.
   8a. **Pre-commit gate (mandatory)** — before any commit, run `pnpm nx affected -t lint test build typecheck --base=origin/main` (use `origin/main`, not `main`: when you commit on a local branch tracking `main`, `--base=main` resolves to the same commit and `affected` returns nothing — silently skipping the gate). Typecheck must be green (commitlint won't catch TS errors). For changes touching `apps/api/`, also run `pnpm nx run api:e2e` locally before pushing — CI integration job will block the PR otherwise. Write tests defensively: unit-test domain logic + handler edge cases, integration-test repositories against real Postgres (Testcontainers), and add an e2e spec for every new endpoint or middleware behavior. Coverage: **all** projects have 60% thresholds via the shared `vitest.shared.ts` factory — lines, functions, branches, statements. The gate is live in CI (`nx affected --target=test … -- --coverage`): vitest exits non-zero on shortfall. Per-project overrides via `coverageThresholds` option (e.g., `api` has `branches: 55` while gaps are being closed). Locally the inferred `@nx/vitest` target omits `--coverage`, so a run that is green on your machine can still fail the PR; reproduce with `pnpm nx test <project> -- --coverage` before pushing.
9. **API contract is fixed** — successful 2xx returns the resource **directly** (Stripe-style, no `{ data, meta }` envelope); list endpoints return `ListResponseDto<T>` via `@ApiPaginatedResponse`. Errors are **RFC 9457 Problem Details** (`application/problem+json`) emitted by the global exception filter — never hand-roll error bodies. For domain violations throw `DomainException` from `@nestjs-fastify-nx/core`; for input validation, the global `ProblemDetailsValidationPipe` already handles it. Decorate controllers with `@ApiCommonErrors` from `@nestjs-fastify-nx/contracts` so Swagger documents the error responses. Naming: camelCase JSON keys, snake_case error `code` values (see `ERROR_CODES`), kebab-case HTTP headers (`X-Request-Id`).
   ...
   - **Errors: pick `kind`, never a status.** `DomainException` is a plain `Error` with **no HTTP status** — the same handlers run under REST, GraphQL and the scheduler's outbox listener, so only the transport can assign one. `GlobalExceptionFilter` maps `kind` → status; GraphQL matches `err.kind`. Full guide + verified examples: `docs/error-handling.md`.

     | `kind`       | Status | Throw it when                                                               |
     | ------------ | ------ | --------------------------------------------------------------------------- |
     | `malformed`  | 400    | Input could not be parsed at all (opaque cursor/token that will not decode) |
     | `validation` | 422    | Input parsed but broke a rule the client can fix — the default              |
     | `not_found`  | 404    | Resource absent, or the caller must not learn it exists                     |
     | `conflict`   | 409    | Duplicate key, stale version, work in flight — pair with `permanent: false` |
     | `forbidden`  | 403    | Authenticated but not permitted                                             |

     `malformed` vs `validation` is RFC 9110 §15.5: 400 = "could not understand", 422 = "understood, and it is wrong". `permanent` defaults to **true** — a rejected rule never becomes valid by waiting, so `OutboxRelayService` parks the row instead of burning 10 retries; set `false` only for a conflict a later attempt could win. `code` MUST come from `ERROR_CODES` (`libs/contracts`), never an ad-hoc string, because the client keys its i18n off it. Never `extends HttpException` in `libs/core`, a domain entity, or an application handler — that pins a transport status onto code the worker and scheduler also run. `501` is what a freshly generated repository throws (`NotImplementedException`); an unmatched method on an existing path answers `404`, not `405` (Fastify router default, not overridden).
   ...
   - **Idempotency**: mutating `/api/v1/*` requests may send `Idempotency-Key: <opaque>` (Stripe pattern). A Nest interceptor plus Fastify `onSend` hook (`apps/api/src/common/idempotency/register-idempotency.ts`, wired in `main.ts` **before** `@fastify/compress` so it stores the uncompressed body; Redis cache DB 5) replays the first 2xx response byte-for-byte with `Idempotent-Replayed: true`. Reuse with a different body → `422 idempotency_key_mismatch`; in-flight duplicate → `409 idempotency_key_conflict`. Acquisition and replay run after all Nest guards; the store scopes by verified session/user/organization or API-key/organization IDs, never raw credentials or IP. Anonymous writes bypass caching. The `onSend` hook captures the final status and body for exact replay. Fail-open on Redis outage. Invariant enforced on boot: `IDEMPOTENCY_LOCK_TTL_SECONDS*1000 > HTTP_REQUEST_TIMEOUT_MS`. See `docs/code-standards.md`.
   - **Request timeout**: global `TimeoutInterceptor` aborts handlers exceeding `HTTP_REQUEST_TIMEOUT_MS` (default 30s) with `504 request_timeout`. WS + hijacked auth routes exempt. Node can't cancel the promise, so orphaned work still runs — only the client wait is bounded.
   - **Upload**: presign-confirm pattern (`POST /api/v1/upload/{presign,confirm}`). Browser uploads bytes directly to S3 via the issued policy; server only HEADs the object on confirm and verifies size + MIME against the magic-byte allow-list in `libs/shared/src/lib/file-signature.ts` (shared with the worker's `upload-verification` processor, which re-checks the final object). `UPLOAD_MAX_FILE_BYTES` is the single source of truth — wired into both `@fastify/multipart` and the controller's policy cap so both layers reject at the same threshold.
   ...
   - **`X-Request-Id` is automatic** — set by `CorrelationIdMiddleware` (NestJS layer) and `fastify-error-handler` (Fastify-level errors before pipeline). Never set it manually in a controller. The incoming `x-request-id` is echoed **only** when `TRUST_INBOUND_REQUEST_ID=true` (default `false`, so a client-supplied id is dropped and a fresh one minted — a caller cannot make unrelated requests share an id); `X-Correlation-Id` is the caller-owned header and is always honoured when it passes the safe-charset check. Any layer that needs the id MUST call `ensureRequestIds(req.raw, req.headers)` from `apps/api/src/common/logging/request-id.ts` — never `resolveRequestId()` directly. The latter mints a fresh random id whenever tracing is off (the default), so a second call for the same request hands the client an `X-Request-Id` that matches no log line. `ensureRequestIds` is resolve-once: first caller stamps the raw request, everyone after reads it back.
...
- **Three dependencies are deliberately held back — do not "fix" them in a bump PR.** `@nestjs/event-emitter` stays on **3.x**. Its 12.0.0 release re-exports the emitter as `typeof eventemitter2.EventEmitter2` from a default import, but `eventemitter2@6.4.9` — the exact version 12.0.0 pins, and the latest published — declares `export default EventEmitter2`, the class rather than a namespace. The re-export therefore resolves to an error type; `skipLibCheck: true` hides the TS2339 at typecheck, so it surfaces as `@typescript-eslint/no-unsafe-*` failing `infra-messaging:lint` on `emitter.emitAsync`. Runtime is unaffected (`require('eventemitter2').EventEmitter2` is a function), so this is a typings-only upstream defect with no released fix; 3.1.0 exposes the same `emitAsync`/`@OnEvent` surface. Dependabot ignores its major. `graphql` stays on **16.x**: `@nestjs/graphql` declares peer `graphql: ^16.11.0` and `mercurius` declares `^16.0.0`, and `strictPeerDependencies: true` makes a 17.x install fail outright. `cookie` stays on **1.x**, pinned by the `pnpm-workspace.yaml` override. There is no longer a direct `cookie` dependency — cookies are parsed by **`@fastify/cookie`** (registered in `main.ts`; idempotency uses verified guard identities, not cookie parsing). `@fastify/cookie@11` declares `cookie: ^2.0.0` but calls only `parseCookie()`/`stringifySetCookie()`, which `1.1.1` already exports, so the override holds it there safely — re-verify that when bumping the plugin. v2 itself is still unreachable: it dropped the `parse`/`serialize` exports, and moving the tree to 2.x makes three `cookie` majors coexist, because `light-my-request` (a Fastify core dep) resolves `1.1.1` and `express`/`engine.io` resolve `0.7.2`. Nx 23.1.0 lockfile pruning then fails for `worker` and `scheduler` with `Target project does not exist: npm:cookie@1.1.1`, swallows the error, writes the FULL workspace lockfile into `dist/apps/<app>/`, and the prod image dies on `ERR_PNPM_OUTDATED_LOCKFILE`. Cause is upstream: `pruneProjectGraph` rehoists a package left with a single version by mutating `node.name` on the object it SHARES with the source graph, so a later prune resolves that node by key yet registers it under the mutated name and the dependency edge no longer resolves. A pnpm override forcing 2.x is NOT a workaround — `light-my-request` calls `cookie.serialize()`, which v2 removed. Dependabot is set to ignore the major (see #134). Revisit when `light-my-request`/Fastify move to cookie v2.
- **pnpm overrides** live in `pnpm-workspace.yaml` (pnpm 11 no longer reads `package.json#pnpm`) and pin transitive vulnerable versions (fastify, find-my-way, picomatch, brace-expansion, protobufjs, …). An override is the ONLY way to patch a dependency a parent pins exactly — e.g. `@nestjs/platform-fastify` pins `find-my-way: "9.6.0"` with no caret, so GHSA-c96f-x56v-gq3h needed `find-my-way: '>=9.7.0'` rather than an update. `minimumReleaseAge: 1440` also means a release younger than 24 h is skipped on purpose; a package showing as outdated right after publish is the policy working, not a bug. **This bites `nx migrate` specifically**: it rewrites `package.json` to the new version, then `pnpm install` silently declines the too-young release and leaves the lockfile behind — exit 0, warning buried in the output, `package.json` and lockfile now out of sync (CI `--frozen-lockfile` and the prod image both fail on it). Always finish a migrate with `pnpm install --frozen-lockfile`: it is the check that surfaces the drift. Trivy may still flag declared ranges in transitive `package.json` files — that's a scanner artifact; runtime resolves to the override. `lefthook` bundles a Go binary; Go stdlib CVEs (e.g. CVE-2026-25679..42499) are only patched when upstream releases a new binary — watch [evilmartians/lefthook releases](https://github.com/evilmartians/lefthook/releases) and bump the devDependency when a patched version ships. **Security-floor overrides MUST be capped, not open-ended `>=`.** `@babel/plugin-transform-modules-systemjs: '>=7.29.4 <8'` — an open `>=7.29.4` let pnpm resolve `8.0.1`, which demands `@babel/core ^8` while the nx toolchain pins `@babel/core 7`, so `strictPeerDependencies` failed every lockfile-updating `pnpm install` (CI's `--frozen-lockfile` masked it; local dev installs broke). Cap any `>=` override below the next major that would move a peer. **`peerDependencyRules.allowedVersions` is the separate, narrower tool** — it does not change what gets installed, it only tells `strictPeerDependencies` that a specific parent's stale peer range is acceptable. The current entries all exist because NestJS 12 shipped before `@nx/nest`, `@sentry/nestjs`, `@nestjs/throttler` and `@nest-lab/throttler-storage-redis` refreshed their metadata; Nest 12 keeps a `require` condition in its exports map, so those CJS consumers still load. Every entry is keyed `parent>peer` so the allowance never leaks to another consumer, and each one must be justified by a green `lint test build typecheck` plus `api:e2e` run — delete it the moment upstream declares the range.
- **Authorization has TWO axes and they must not be mixed.**
  - **Platform axis** — `User.role` (`ADMIN`/`USER`), gated by `@Roles()` + `RolesGuard`. This is the _provider's own staff_, not the customer. Use it only for back-office surfaces that legitimately span tenants.
  - **Organization axis** — permissions resolved for `(user, activeOrganization)`, gated by `@RequirePermission()` + `PermissionGuard` (`@nestjs-fastify-nx/infra-authorization`). Every tenant-facing endpoint uses this one.
...
- **Client error output is allowlisted, never copied from thrown errors.** Every 5xx is generic in every environment across Nest REST, direct Fastify, Better Auth (`reply.hijack()`), GraphQL, WebSocket handshakes, and Bull Board. A `messageKey` does not bypass 5xx masking. Validation/business `errors[]` never echo the rejected value and response builders strip unknown extension fields (`stack`, `cause`, `internal`, `received`, etc.). Full causes belong only in structured logs/Sentry under `requestId`. Prisma has a final safety-net in `GlobalExceptionFilter`: P2002/P2003→409, P2025→404, P2023→400, P2028/P2024→503; every other `PrismaClient*` error becomes a generic 500. Prefer explicit domain handling for expected conditions.
- **CSP is strict globally, relaxed only for Bull Board.** Production CSP has no `'unsafe-inline'` on `script-src`/`style-src`. Bull Board's bundled UI needs inline script/style (its HTML isn't ours to nonce), so `main.ts` overrides CSP for `/api/admin/queues` via a scoped `onSend` hook. Do not add `'unsafe-inline'` back to the global policy — scope any HTML-surface exception to its path.
- **`@Cron` tasks are not serialized by `@nestjs/schedule`** — a run that outlasts its interval overlaps the next tick. Every cron handler in `apps/scheduler` guards with a private `running` boolean (one per method) in addition to the `leadership.isLeader()` gate, mirroring `OutboxRelayService`. Add the same guard to any new cron.
- **BullMQ jobs carry `correlationId` from the enqueuing request.** Request-scoped producers (e.g. `UploadController`) read it via `ClsService.get(REQUEST_CONTEXT_KEYS.correlationId)` and put it on `job.data`; processors log it so worker logs correlate to the origin request. Outbox-originated jobs (welcome-email) have no request context — they are traced by the outbox `eventId` in the jobId instead.
- **Node default metrics (GC/eventloop/CPU) are collected ONCE per process** onto a module-level shared registry in `MetricsService`; `render()` merges it with the instance registry and re-applies `app="api"` (`Registry.merge` drops per-registry default labels). The metrics client's GC `PerformanceObserver` has no public `disconnect`, so re-collecting per module lifecycle would leak an observer each time (notably across tests). Don't move `collectDefaultMetrics` back into `onModuleInit` on the instance registry.
- **Global enhancers MUST bypass the `ws` context.** The three `APP_GUARD`s (`GqlThrottlerGuard`, `BetterAuthGuard`, `RolesGuard`) and `TimeoutInterceptor` all run on `@SubscribeMessage` handlers too. WebSocket auth/rate-limit is done at the socket.io layer instead — `createWsAuthMiddleware` (session + per-IP connection lease), periodic revalidation, and `applyWsMessageRateLimit` (per-socket inbound frame budget, `WS_MESSAGE_RATE_LIMIT_MAX` / `WS_MESSAGE_RATE_LIMIT_WINDOW_MS`) — so each returns early on `context.getType() === 'ws'`. Without it the throttler reads the socket as an HTTP `res` and throws `res.header is not a function` on every message. Any new global guard/interceptor must add the same `ws` guard, and any new `@SubscribeMessage` handler is covered by the socket-level budget, not by `@Throttle`.
- **Repositories are transaction-aware.** They resolve `this.prisma.currentTransaction ?? this.prisma.db` (writes) / `?? this.prisma.dbRead` (reads), NOT `this.prisma.db` directly — so a repository write issued inside `prisma.transaction(...)` commits atomically with the outbox row a command handler publishes there (the documented producer pattern). Copy that accessor when adding a repo. `prisma.transaction()` also throws on nesting (a nested call would open a second, non-atomic physical tx).
- **Account-wide auth limiter matches on the RESOLVED route, not the raw URL.** The credential-stuffing bucket in `main.ts` keys off `req.routeOptions?.url` against `STRICT_AUTH_PATHS`, and the adapter runs with `ignoreTrailingSlash: true`. A raw-string check on `req.url` was bypassable via percent-encoding (`/sign-in/%65mail`) or a trailing slash (falls to the looser wildcard). Don't revert to string-matching `req.url`.
- **`injectDatabasePassword` encodes the secret with `encodeURIComponent`** before the WHATWG-URL password setter. The setter does NOT percent-encode a literal `%`, so a password containing `%` would serialize to an ambiguous `%XX` that libpq/pg-connection-string mis-decode → wrong password → boot-time DB auth failure. It also can't parse comma-separated multi-host DSNs (rare) — those pass through unchanged. `apps/migration/src/main.ts` keeps a deliberate COPY of this function (scope:migration has an empty boundary allow-list and can't import shared) — keep the two in sync.
- **Per-app env validators (`apps/{api,worker,scheduler}/src/config/env.validation.ts`) must stay in PARITY for shared-concern production checks.** They validate separately but a prod deploy may give each app its own env, so a check that matters to a runtime (e.g. `MAIL_HOST != localhost` matters to the worker that actually sends mail; `EVENT_PUBLISHER_DRIVER=outbox` matters to api + scheduler) must live in EACH validator that runs that concern — a gap lets one process boot green and fail at runtime. When adding a production `superRefine` rule, add it to every validator whose app is affected.
- **Health probes share `withTimeout` from `@nestjs-fastify-nx/shared`** (was copy-pasted in three indicators). It bounds the wait only — the underlying promise is not cancelled. Reuse it (don't re-inline a Promise.race+setTimeout) for any new probe.
...
- **Fastify-level errors are normalized by an `onSend` hook, NOT a second error handler.** `applyFastifyProblemDetailsHook()` (`main.ts`) rewrites any response with status ≥ 400 into RFC 9457 problem+json, so body-parser / multipart / `FST_ERR_*` failures match the shape the global exception filter emits. It is deliberately a hook and not `setErrorHandler`: NestJS already installs one (`routes-resolver.registerExceptionHandler()`), which routes every Fastify-level error into `GlobalExceptionFilter` with i18n + CLS context. Overriding it would drop both and trigger FSTWRN004. Do not reintroduce a competing `setErrorHandler` on the root scope.
- **Plugins that own their own scope may install their own error handler.** `@bull-board/fastify` does (`FastifyAdapter.registerPlugin()`), so nothing raised inside `/api/admin/queues` ever reaches Nest's handler — its default collapses everything to `error.statusCode || 500`, which silently turns an RFC 9457 error (carrying `status`, not `statusCode`) into a 500. `createBullBoardPlugin` overrides it via the adapter's public `setErrorHandler()`. When mounting any other third-party Fastify plugin, check whether it calls `setErrorHandler` before assuming the global contract applies.
- **Never statically import a `devDependency` from a runtime code path.** Nx `generatePackageJson` (webpack) copies only root `dependencies` into the generated `dist/apps/<app>/package.json`; devDeps are dropped. The prod image runs `pnpm install --prod` against that file and webpack externalizes node_modules, so a top-level `import { X } from '<devDep>'` becomes a runtime `require` of a module that isn't installed → the container **crashes on boot** (`MODULE_NOT_FOUND`), even if the code that uses it is gated behind `if (!isProduction)`. The gate stops the _call_, not the _module load_. Fix: `const { X } = await import('<devDep>')` inside the dev-only function (see `swagger.config.ts` → `@apiture/openapi-down-convert`, loaded lazily inside `mergeBetterAuthSpec` because Swagger is dev/codegen-only). Every image — dev included — installs `--prod` from the generated manifest, so the CI `docker-smoke` job boots the dev stack and catches this on the PR; unit tests and Testcontainers e2e run against the source tree and cannot. Always boot the prod Docker image once when touching `main.ts`'s static import graph.
...
- **Better Auth email flows are FE-owned UI, BE-owned validation.** Backend wires `sendResetPassword` + `emailVerification.sendVerificationEmail` + `user.deleteUser.sendDeleteAccountVerification` in `libs/infra/auth/src/lib/better-auth.config.ts`. Each callback enqueues a BullMQ job on `QUEUE_NAMES.EMAIL_NOTIFICATION`; the worker process owns SMTP. The link inside the email points at **`FRONTEND_BASE_URL`** — the SPA host that renders these pages:
  ...
  Organization invitations follow the same shape: `sendInvitationEmail` links to `GET /accept-invitation?id=<invitationId>`, and the SPA submits `POST /api/auth/organization/accept-invitation { invitationId }`.

  **Accepting an invitation requires a verified mailbox, and that is deliberate.** Because `advanced.database.generateId` is `false`, Better Auth treats the invitation id as database-generated (therefore potentially guessable) and refuses accept/reject from an unverified account — otherwise anyone who guessed an id could register with the invitee's address and join the organization. Do NOT "fix" a 403 here with `requireEmailVerificationOnInvitation: false`; verify the mailbox instead (`apps/api/e2e/organizations.e2e-spec.ts` shows the flow).
...
- **Outbox has TWO write paths — neither is `eventEmitter.emit()` in a command handler.** All domain events land in `outbox_events`; `OutboxRelayService` drains it (poll interval `OUTBOX_POLL_INTERVAL_MS`, batch `OUTBOX_BATCH_SIZE`, listener tx budget `OUTBOX_TX_TIMEOUT_MS` default 30s). Failed rows back off exponentially — `min(OUTBOX_RETRY_BASE_MS × 2^attempts, OUTBOX_RETRY_MAX_MS)`, stamped on `outbox_events.nextAttemptAt` **by the CLAIM statement itself** so the backoff survives a relay that dies mid-dispatch. Do not move that stamp into the error handler, and do not remove it: without backoff the relay re-claims a failing row every poll tick, so a downstream outage of only `OUTBOX_MAX_ATTEMPTS × OUTBOX_POLL_INTERVAL_MS` (10 s at the defaults) burns the whole retry budget of every pending event and parks them all as permanently stuck. The two producer paths:
  1. **Postgres AFTER INSERT/DELETE triggers** (`prisma/migrations/20260501000000_init/migration.sql`) — used for every Better Auth-driven event: `users.registered`, `users.logged_in`, `users.logged_out`, plus `organizations.created`, `organizations.deleted`, `organizations.member_added`, `organizations.member_removed`. Reason: Better Auth commits its writes outside the NestJS request pipeline, so a Nest-side hook would race the commit and could lose events on crash. The trigger fires inside the same tx as the source row — both commit or neither. Payload shape mirrors `OutboxPublisher.serializePayload()` so the relay rebuilds the in-memory event without a special path. The `organizations.*` triggers also stamp `outbox_events.organizationId`, which the relay reads into `DomainEvent.organizationId` so a listener in another process can rebuild the tenant context. Two deliberate asymmetries: `organizations.deleted` leaves that column NULL (the organization no longer exists, and a dangling id would invite a lookup), and `member_removed` is skipped when the parent organization is being deleted (the cascade would otherwise emit one row per member and bury the single event that describes what happened).
  ...
  - The name goes in `DOMAIN_EVENTS` (`@nestjs-fastify-nx/shared`) — never a string literal. It is a
    published contract: written by trigger SQL, matched by `@OnEvent` in another process, and stored
    in `outbox_events` rows that outlive the deploy. `domain-events.spec.ts` fails if a name drifts
    from the migration SQL; renaming one requires a migration.
  - Listeners receive a **plain `DomainEvent`** rebuilt from the row, never the domain event class
    instance — type the parameter `DomainEvent` and `safeParse` the payload (see
    `userEventPayloadSchema`). Casting compiles and then loses data silently at runtime.
...
- `docs/error-handling.md` — which error to throw, which status the client gets, what never leaks
- `docs/scaling-to-services.md` — what is ready for a service split, what blocks it, and the order to do it
