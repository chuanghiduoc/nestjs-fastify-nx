...
2. **Comments — default to none.** Write one only when the code cannot carry the information itself: a non-obvious constraint, a rejected alternative, or an upstream bug/quirk being worked around. Then keep it to **1–2 lines**; if it needs a paragraph, the explanation belongs in `docs/` or the PR description, not the file. Never narrate what the code does, never restate a name, never reference task IDs, callers, or "added for X". Prefer a well-named function or a test over a comment. When editing existing code, delete comments that no longer earn their place instead of extending them.
