...
8. **Module boundaries are sacred** — if lint fails on `@nx/enforce-module-boundaries`, fix the architecture, do not relax the rule.
   8a. **Pre-commit gate (mandatory)** — before any commit, run `pnpm nx affected -t lint test build typecheck --base=origin/main` (use `origin/main`, not `main`: when you commit on a local branch tracking `main`, `--base=main` resolves to the same commit and `affected` returns nothing — silently skipping the gate). Typecheck must be green (commitlint won't catch TS errors). For changes touching `apps/api/`, also run `pnpm nx run api:e2e` locally before pushing — CI integration job will block the PR otherwise. Write tests defensively: unit-test domain logic + handler edge cases, integration-test repositories against real Postgres (Testcontainers), and add an e2e spec for every new endpoint or middleware behavior. Coverage target: aim for ≥ 60 % across layers. NOTE: the `thresholds` in each `vitest.config.mts` are the **target, not a live gate** — the inferred `@nx/vitest` target runs without `--coverage`, so nothing fails on low coverage today, and some projects (e.g. composition-admin, modules-upload) currently sit near 0 %. Treat coverage as debt to pay down, not a passing check; raising it and turning the thresholds into a real gate is tracked separately.
9. **API contract is fixed** — successful 2xx returns the resource **directly** (Stripe-style, no `{ data, meta }` envelope); list endpoints return `ListResponseDto<T>` via `@ApiPaginatedResponse`. Errors are **RFC 9457 Problem Details** (`application/problem+json`) emitted by the global exception filter — never hand-roll error bodies. For domain violations throw `BusinessRuleException` from `@nestjs-fastify-nx/core`; for input validation, the global `ProblemDetailsValidationPipe` already handles it. Decorate controllers with `@ApiCommonErrors` from `@nestjs-fastify-nx/contracts` so Swagger documents the error responses. Naming: camelCase JSON keys, snake_case error `code` values (see `ERROR_CODES`), kebab-case HTTP headers (`X-Request-Id`).
   ...
   - **`X-Request-Id` is automatic** — set by `CorrelationIdMiddleware` (NestJS layer) and `fastify-error-handler` (Fastify-level errors before pipeline). Never set it manually in a controller; it always echoes the incoming `x-request-id` or mints a fresh one. Any layer that needs the id MUST call `ensureRequestIds(req.raw, req.headers)` from `apps/api/src/common/logging/request-id.ts` — never `resolveRequestId()` directly. The latter mints a fresh random id whenever tracing is off (the default), so a second call for the same request hands the client an `X-Request-Id` that matches no log line. `ensureRequestIds` is resolve-once: first caller stamps the raw request, everyone after reads it back.
