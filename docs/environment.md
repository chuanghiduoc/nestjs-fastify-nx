...
| `API_DB_USER`                     | `api_user`                                                | Docker only | Role the api connects as. `NOBYPASSRLS` — the request path is RLS-enforced                               |
| `API_DB_PASSWORD`                 | `api_dev_password`                                        | Docker only | Password for the above; `gen-env.sh` generates a real one                                                |
| `WORKER_DB_USER`                  | `worker_user`                                             | Docker only | Role the worker connects as. `BYPASSRLS`, DML on `stored_files` only                                     |
| `WORKER_DB_PASSWORD`              | `worker_dev_password`                                     | Docker only | Password for the above; `gen-env.sh` generates a real one                                                |
| `SCHEDULER_DB_USER`               | `scheduler_user`                                          | Docker only | Role the scheduler connects as. `BYPASSRLS`, `MAINTAIN` plus DML on the tables its jobs touch            |
| `SCHEDULER_DB_PASSWORD`           | `scheduler_dev_password`                                  | Docker only | Password for the above; `gen-env.sh` generates a real one                                                |
| `POSTGRES_ADMIN_USER`             | _(unset)_                                                 | Prod only   | Schema owner used by `db-grants` and the migration one-shot. Dev reuses `POSTGRES_USER`                  |
| `POSTGRES_ADMIN_PASSWORD`         | _(unset)_                                                 | Prod only   | Password for the above                                                                                   |
...
| `REDIS_CACHE_PASSWORD`       | `cache_dev_password` | In prod  | `requirepass` for the cache instance. Only the api receives it. Blank leaves the local instance unauthenticated; api env validation refuses to boot in production without it |
| `REDIS_QUEUE_HOST`           | `localhost`          | Yes      | Queue Redis hostname                                                                                                                                                         |
| `REDIS_QUEUE_PORT`           | `6380`               | Yes      | Queue Redis port                                                                                                                                                             |
| `REDIS_QUEUE_PASSWORD`       | `queue_dev_password` | In prod  | `requirepass` for the queue instance. Issued to api, worker and scheduler. Required in production by all three validators                                                    |
...
| `TZ`                  | `UTC`                   | No       | IANA timezone resolved by Node (requires `tzdata` in the image). Postgres is pinned to UTC in compose regardless — the schema stores naive timestamps |
...
| `OUTBOX_PARKED_RETENTION_DAYS` | `30`        | No       | Age after which a **parked** row (attempts exhausted, never delivered) is dropped by the 03:50 UTC cron, logging `eventType`/`lastError`. Validated at boot to be ≥ `OUTBOX_RETENTION_DAYS`  |
