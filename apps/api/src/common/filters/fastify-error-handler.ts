...
export function applyFastifyErrorHandler(fastify: FastifyInstance): void {
  fastify.setErrorHandler((error: FastifyError, request: FastifyRequest, reply: FastifyReply) => {
    ...
    const status = resolveFastifyStatus(error);
    const code = resolveFastifyCode(error, status);
    ...
    const requestId =
      ...
      resolveRequestId(request.headers);
    const correlationId =
      clsStore?.correlationId || resolveCorrelationId(request.headers, requestId);
