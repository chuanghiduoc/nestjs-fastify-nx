...
// 24h covers the BullMQ stalled-recovery window (3 attempts × exponential backoff).
...
export class EmailNotificationProcessor extends WorkerHost {
  ...
  async process(job: Job<EmailNotificationPayload>): Promise<void> {
    ...
    // SETNX: BullMQ retries reuse job.id — claimed slot means exactly one delivery per job.
    ...
    try {
      ...
      // Clear idempotency marker so retry can proceed. If del fails, marker lives 24h and email is permanently skipped.
      await this.redis.del(key).catch((delErr) => {
        this.logger.error(
          { jobId: job.id, recipient: toMasked, cause: delErr },
          `SETNX marker cleanup failed after SMTP error — email may be permanently skipped. jobId=${job.id} recipient=${toMasked}`,
        );
      });
      this.logger.error(
